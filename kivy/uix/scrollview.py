...
class ScrollView(StencilView):
    ...
    always_enable_overscroll = BooleanProperty(False)
    ...
    :attr:`always_enable_overscroll` is a
    ...
    def _update_effect_x_bounds(self, *args):
        if not self._viewport or not self.effect_x:
            return
        self.effect_x.min = min(0, self.width - self.viewport_size[0])
        self.effect_x.max = 0
    ...
    def _update_effect_y_bounds(self, *args):
        if not self._viewport or not self.effect_y:
            return
        self.effect_y.min = self.height - self.viewport_size[1]
        self.effect_y.max = max(0, self.height - self.viewport_size[1])
    ...
    def on_scroll_start(self, touch, check_children=True):
        ...
        width_scrollable = self.always_enable_overscroll or vp.width > self.width
        height_scrollable = self.always_enable_overscroll or vp.height > self.height
            ...
            if e:
                if btn in ('scrolldown', 'scrollleft'):
                    if self.smooth_scroll_end:
                        ...
                        if not self.always_enable_overscroll:
    ...
    def update_from_scroll(self, *largs):
        ...
        if vp.width > self.width or self.always_enable_overscroll:
            sw = vp.width - self.width
            x = self.x - self.scroll_x * sw
        else:
            x = self.x

        if vp.height > self.height or self.always_enable_overscroll:
            sh = vp.height - self.height
